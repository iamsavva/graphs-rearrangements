...
from graph_tsp_gcs import VertexTSP, EdgeTSP, VertexTSPprogram, EdgeTSPprogram
...
class GraphTSPGCSProgram:
    ...
    def add_program_vertex(self, tsp_vertex: VertexTSP):
        assert tsp_vertex.name not in self.vertices, "Vertex with name " + tsp_vertex.name + " already exists"
    ...
    def add_program_tsp_edge(self, e: EdgeTSP):
        ...
        self.edges[edge_name] = EdgeTSPprogram(self.vertices[e.left], self.vertices[e.right], edge_name)
    ...
    def add_tsp_variables_to_prog(self) -> None:
        ...
        for v in self.vertices.values():
            # visitation variable
            v.set_v(self.prog.NewContinuousVariables(self.num_possible_objects, "v_" + v.name))
        ...
        for e in self.edges.values():
            ...
            e.set_left_v(self.prog.NewContinuousVariables(self.num_possible_objects, "left_v_" + e.name))
            e.set_right_v(self.prog.NewContinuousVariables(self.num_possible_objects, "right_v_" + e.name))
            # left and right order
    ...
    def add_tsp_constraints_to_prog(self) -> None:
        ...
        for v in self.vertices.values():
            ...
            if v.name == "start":
                ...
                self.prog.AddLinearConstraint(eq(v.v, np.zeros(self.num_possible_objects)))
                # visit continuity
                ...
                self.prog.AddLinearConstraint(eq(v.v, np.ones(self.num_possible_objects)))
...
# TODO
    ...
    def add_motion_planning(self) -> None:
        ...
        for gcs_edge in self.gcs_edges:
            MotionPlanningProgram(
                ...
                start_vertex = None,
                target_vertex = None,
                set_tessellation = None, # already a graph?
                options = None,
            )
            MotionPlanning(
                prog=self.prog,
    ...
    def solve(self):
        ...
        v_path, e_path = self.find_path_to_target(non_zero_edges, self.vertices["start"])
